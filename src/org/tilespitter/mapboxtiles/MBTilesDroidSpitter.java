...
import org.tilespitter.mapboxtiles.MbTilesMetadata.MetadataParseException;
import org.tilespitter.mapboxtiles.MbTilesMetadata.MetadataValidator;
...
//	private MbTilesSQLite mbtilesdb;
	...
	public MBTilesDroidSpitter(Context ctx, File dbpath) {
	...
	public Bitmap getTileAsBitmap(String x, String y, String z) {
		...
		Cursor c = db.rawQuery("select tile_data from tiles where tile_column=? and tile_row=? and zoom_level=?", new String[]{x, y, z});
		if (!c.moveToFirst()) {
			c.close();
			return null;
		}
	...
	public MbTilesMetadata fetchMetadata(String metadataVersion) throws MetadataParseException {
		Cursor c = db.query(MbTilesSQLite.TABLE_METADATA,
				new String[] {MbTilesSQLite.COL_METADATA_NAME, MbTilesSQLite.COL_METADATA_VALUE}, 
				null, null, null, null, null);
		MetadataValidator validator = MbTilesMetadata.MetadataValidatorFactory.getMetadataValidatorFromVersion(metadataVersion);
		if (validator == null)
			return null;
		
		this.metadata = MbTilesMetadata.createFromCursor(c, 
				c.getColumnIndex(MbTilesSQLite.COL_METADATA_NAME), 
				c.getColumnIndex(MbTilesSQLite.COL_METADATA_VALUE),
				validator
		);
